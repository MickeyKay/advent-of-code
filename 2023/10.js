...
const map = getParsedInputLines(REAL_INPUT).map(val => val.split(''));

const getAvailableCoordinates = (y, x) => {
  let coordinates = [];
  ...
  try {
    ...
    console.log('Out of bound, working as expecte:', e);
  }
...
const testValidateLogic = () => {
  map.forEach((row, yIndex) => {
    row.forEach((char, xIndex) => {
      if ('.' != char) {
        const [coordsOne, coordsTwo] = getAvailableCoordinates(yIndex, xIndex);
        console.log(char, ' => ', map[coordsOne[0]][coordsOne[1]], map[coordsTwo[0]][coordsTwo[1]], '\n');
      }
    })
  });
}
...
const getPartOneSolution = () => {
  ...
  map.forEach((row, yIndex) => {
    row.forEach((char, xIndex) => {
      if ('S' == char) {
        startCoord = [yIndex, xIndex];
      }
    })
  });

  // Find first path tile.
  const [y, x] = startCoord;
  let step = [];

  // 1. Up.
  if (JSON.stringify(getAvailableCoordinates(y-1, x)).includes(`[${y},${x}]`)) {
    step = [y-1, x];
  }

  // 2. Down.
  else if (JSON.stringify(getAvailableCoordinates(y+1, x)).includes(`[${y},${x}]`)) {
    step = [y+1, x];
  }

  // 3. Left.
  else if (JSON.stringify(getAvailableCoordinates(y, x-1)).includes(`[${y},${x}]`)) {
    step = [y, x-1];
  }
  ...
  let steps = [startCoord];
  let currentStep = startCoord;

  while (map[step[0]][step[1]] != 'S') {
    steps.push(step);
    const nextAvailableCoords = getAvailableCoordinates(step[0], step[1]);
    //console.log('======\n', step, '\n', nextAvailableCoords,'\n\n');
    if (
      nextAvailableCoords[0][0] == currentStep[0]  &&
      nextAvailableCoords[0][1] == currentStep[1]
    ) {
      currentStep = step;
      step = nextAvailableCoords[1];
    } else {
      currentStep = step;
      step = nextAvailableCoords[0];
    }
  }
