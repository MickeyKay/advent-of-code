...
const getPartOneSolution = input => {
  const parsedLines = getParsedInputLines(input).filter(val => val);
  const instructions = parsedLines.shift();

  const nodes = {}

  parsedLines.forEach(line => {
    const lineParts = line.match(/(.*)\s=\s\((.*),\s(.*)\)/);
    nodes[lineParts[1]] = {
      L: lineParts[2],
      R: lineParts[3],
    }
  ...
  while (currentNode !== 'ZZZ') {
    const currentInstruction = instructions.charAt(steps % instructions.length);

    currentNode = nodes[currentNode][currentInstruction]
    //console.log(currentInstruction, currentNode);
    steps++;
  }
...
const getPartTwoSolution = input => {
  ...
  while (true) {
    ...
    // Check if we're all __Z, and break if so.
    currentNodes.forEach((node, index) => {
      currentNodes[index] = nodes[node[currentInstruction]];

      if (node.value.charAt(2) != 'Z') {
        complete = false;
      }
    });
