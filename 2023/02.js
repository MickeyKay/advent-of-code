 ...
 *
 * Questions:
 *   - Can we assume "Game N" string is such that N == index + 1 reliably? Let's assume not, just in case.
 */
...
// Generate color:count reveal object.
const parseRevealIntoColorCountObject = reveal => {
  const spacelessReveal = reveal.replace(' ', '');
}

const parseGameComponents = line => {
  const game = {};
  game.reveals = [];

  const [indexString, revealsString] = line.split(':');

  // Get game index.
  game.index = parseInt(indexString.match(/\d+/g)[0]);

  // Get game reveals array.
  const reveals = revealsString.split(';');

  reveals.forEach(reveal => {
    const revealParts = reveal.match(/(\d)+\s([a-z]+)/g);
    const revealObject = {};

    // I'm not good at regex :|, so nested string handling.
    revealParts.forEach(revealPart => {
      const [count, color] = revealPart.split(' ');
      revealObject[color] = parseInt(count);
    });
    game.reveals.push(revealObject);
  });
 ...
 * PART ONE
 */
const getSumOfIdsPartOne = input => {
  const lines = getParsedInputLines(input);
  var idsSum = 0;

  lines.forEach(line => {
    var violatesParameters = false;
    const parsedGame = parseGameComponents(line);
    parsedGame.reveals.forEach(reveal => {
      for (const color in GAME_CONSTRAINTS) {
        if (GAME_CONSTRAINTS[color] < reveal[color]) {
          violatesParameters = true;
          return;
        }
      }
    });

    if (!violatesParameters) {
      idsSum += parsedGame.index;
    }
...
// console.log(getSumOfIdsPartOne(REAL_INPUT));

/**
 * PART TWO
 */
const getSumOfPowerOfMinimumCubesPartTwo = input => {
  const lines = getParsedInputLines(input);
  var powersTotal = 0;

  lines.forEach(line => {
    const parsedGame = parseGameComponents(line);
    const minCountObject = {
        red: 0,
        blue: 0,
        green: 0,
      };
    parsedGame.reveals.forEach(reveal => {
      for (const color in GAME_CONSTRAINTS) {
        const revealColorCount = reveal[color] ?? 0;
        minCountObject[color] = minCountObject[color] < revealColorCount ? revealColorCount : minCountObject[color];
      }

    });

    powersTotal += minCountObject.red * minCountObject.blue * minCountObject.green;
...
console.log(getSumOfPowerOfMinimumCubesPartTwo(REAL_INPUT));
