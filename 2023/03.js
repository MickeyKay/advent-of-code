...
const arrayContainsSymbol = array => {
  return array.find(isSymbol);
}
...
const getPartsSumPartOne = input => {
  ...
  inputArray.forEach((line, lineIndex) => {
    ...
    line.forEach((char, charIndex)=> {
      ...
      if (isDigit(char)) {
        numberValue += char;
        startIndex = startIndex ?? charIndex;
        endIndex = charIndex;
      } else {
        numberValue && numbersArray.push({
      ...
      if (numberValue && charIndex == line.length -1) {
        numbersArray.push({
          value: parseInt(numberValue),
          startIndex,
          endIndex,
...
const addNumberToGear = (lineIndex, charIndex, number) => {
  ...
  if (!potentialGears[potentialGearsIndex]) {
    potentialGears[potentialGearsIndex] = [];
  }
