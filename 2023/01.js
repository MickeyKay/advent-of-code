...
const getCalibrationNumberPartOne = input => {
  const lineCalibrationNumbers = [];

  const linesArray = getParsedInputLines(input);
  ...
  linesArray.forEach(line => {
    ...
    lineCalibrationNumbers.push(parseInt(`${firstDigit}${lastDigit}`));
  });

  return lineCalibrationNumbers.reduce((partialSum, a) => partialSum + a, 0);
}

// console.log(getCalibrationNumberPartOne(REAL_INPUT));


// PART TWO
// =========

const DIGITS_STRING_ENUM = [
  'one',
  'two',
  'three',
  'four',
...
// Note: Not performant to run separate first/last searches, but
// simplest in this context and size of test set is small enough that
// performance is not a concern.

const getFirstDigitOfLine = line => {
  var firstStringIndex = null;
  var firstDigit = null;

  // Parse strings indices.
  DIGITS_STRING_ENUM.forEach((digitString, digitStringIndex) => {
    const digitStringPosition = line.indexOf(digitString);

    // Skip to next iteration if substring not found.
    if (digitStringPosition === -1) {
      return;
    }

    if (firstStringIndex === null || digitStringPosition < firstStringIndex ) {
      firstStringIndex = digitStringPosition;
      firstDigit = digitStringIndex + 1;
    }
  });

  // Parse number indices.
  line.split('').forEach((char, numCheckindex) => {

    // Is a numberic char.
    if (!isNaN(parseInt(char, 10))) {

      if (firstStringIndex === null || numCheckindex < firstStringIndex ) {
        firstStringIndex = numCheckindex;
        firstDigit = char;
      }
    }
  });
...
const getLastDigitOfLine = line => {
  var lastStringIndex = null;
  var lastDigit = null;

  DIGITS_STRING_ENUM.forEach((digitString, digitStringIndex) => {
    const digitStringPosition = line.lastIndexOf(digitString);
    ...
    if (lastStringIndex === null || digitStringPosition > lastStringIndex ) {
      lastStringIndex = digitStringPosition;
      lastDigit = digitStringIndex + 1;
    }
  });
  ...
  line.split('').forEach((char, numCheckindex) => {
    ...
    if (!isNaN(parseInt(char, 10))) {
      ...
      if (lastStringIndex === null || numCheckindex > lastStringIndex ) {
        lastStringIndex = numCheckindex;
        lastDigit = char;
      }
    }
  });
...
const getCalibrationNumberPartTwo = input => {
  const lineCalibrationNumbers = [];
...
console.log(getCalibrationNumberPartTwo(REAL_INPUT));